#!/usr/bin/env python3
"""
build-word-crossword.py — generate the encrypted Word Crossword page.
...
iterations) into:
    ...
    teaching/vocabulary/crossword.md   (layout: protected-crossword)

The game engine (review deck, puzzle generator, grid + clue UI) lives in
_layouts/protected-crossword.html — only the word data is in the encrypted
blob, so editing the game never requires re-encryption. Puzzles are laid
out in the browser at play time; the builder only ships the word records.
...
Usage:
    python3 _tools/build-word-crossword.py "<passphrase>"
"""
...
OUT = os.path.join(ROOT, "teaching", "vocabulary", "crossword.md")
...
def main():
    if len(sys.argv) != 2:
        print('Usage: python3 _tools/build-word-crossword.py "<passphrase>"',
              file=sys.stderr)
        sys.exit(1)
