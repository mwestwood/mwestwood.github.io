#!/usr/bin/env python3
"""
build-word-dash.py — generate the encrypted Meaning Dash page.

Reads _tools/maze_data.py (the same tiers Word Maze and Word Snake use),
packs the two word tiers as compact JSON, encrypts it with the site scheme
(salt[16] + iv[12] + AES-256-GCM ciphertext+tag, PBKDF2-HMAC-SHA256 100k
iterations), and writes:

    teaching/vocabulary/dash.md   (layout: protected-dash)

The game engine (falling word lanes, left/right dashing, lives) lives in
_layouts/protected-dash.html — only the word data is in the encrypted
...
Usage:
    python3 _tools/build-word-dash.py "<passphrase>"
"""
...
OUT = os.path.join(ROOT, "teaching", "vocabulary", "dash.md")
...
def main():
    if len(sys.argv) != 2:
        print('Usage: python3 _tools/build-word-dash.py "<passphrase>"',
              file=sys.stderr)
        sys.exit(1)
