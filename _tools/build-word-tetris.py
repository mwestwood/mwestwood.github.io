#!/usr/bin/env python3
"""
build-word-tetris.py — generate the encrypted Word Tetris page.
...
iterations) into:
    ...
    teaching/vocabulary/tetris.md   (layout: protected-tetris)

The game engine (falling blocks, review deck, word-check quizzes) lives in
_layouts/protected-tetris.html — only the word data is in the encrypted
...
Usage:
    python3 _tools/build-word-tetris.py "<passphrase>"
"""
...
OUT = os.path.join(ROOT, "teaching", "vocabulary", "tetris.md")
...
def main():
    if len(sys.argv) != 2:
        print('Usage: python3 _tools/build-word-tetris.py "<passphrase>"',
              file=sys.stderr)
        sys.exit(1)
